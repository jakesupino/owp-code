...
% % MIXED EFFECTS MODEL
% allSites_mdl_norm_MIXED.all.GPP_avg = fitlme(data_norm.all,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site*season'); % Gives same thing as fitlm
% 
% allSites_mdl_norm_MIXED2.all.GPP_avg = fitlme(data_norm.all,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + season*site + (site|season)');
...
for k = 1:length(predNames)
    meanVal.all.(predNames{k}) = mean(data_norm.all.(predNames{k}),'omitmissing');
    meanVal.winter.(predNames{k}) = mean(data_norm.winter.(predNames{k}),'omitmissing');
    meanVal.spring.(predNames{k}) = mean(data_norm.spring.(predNames{k}),'omitmissing');
    meanVal.summer.(predNames{k}) = mean(data_norm.summer.(predNames{k}),'omitmissing');
    meanVal.fall.(predNames{k}) = mean(data_norm.fall.(predNames{k}),'omitmissing');
    % meanVal.all.(predNames{k}) = mean([meanVal.winter.(predNames{k}) meanVal.spring.(predNames{k}) meanVal.summer.(predNames{k}) meanVal.fall.(predNames{k})]);
end
...
for i = 1:length(respNames)   % Loop through plots for GPP, ER, and NEM
    ...
    for j = 1:length(predNames) % Loop through plotting each predictor
        ...
        x = data_norm.all.(predNames{j});

        % Winter
        ax1 = nexttile(t1);
        plot(data_norm.all.(predNames{j}),data_norm.all.(respNames{i}),'.','color',rgb('lightgrey'))   % Plot All data in grey
        hold on
        ...
        plot(data_norm.winter.(predNames{j}),data_norm.winter.(respNames{i}),'.k')
        ...
        % Spring
        ax2 = nexttile(t1);
        plot(data_norm.all.(predNames{j}),data_norm.all.(respNames{i}),'.','color',rgb('lightgrey'))
        hold on
        % Plot regression line for All data in grey
        switch predNames{j}
            case 'salinity'
        ...
        plot(data_norm.spring.(predNames{j}),data_norm.spring.(respNames{i}),'.k')
        ...
        plot(data_norm.summer.(predNames{j}),data_norm.summer.(respNames{i}),'.k')
        ...
        plot(data_norm.fall.(predNames{j}),data_norm.fall.(respNames{i}),'.k')
...
% % (1) By season
% % GPP
% allSites_mdl.winter.GPP = fitlm(data.winter,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.spring.GPP = fitlm(data.spring,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.summer.GPP = fitlm(data.summer,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.fall.GPP = fitlm(data.fall,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% % ER
% allSites_mdl.winter.ER = fitlm(data.winter,'ER_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.spring.ER = fitlm(data.spring,'ER_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.summer.ER = fitlm(data.summer,'ER_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.fall.ER = fitlm(data.fall,'ER_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% % NEM
% allSites_mdl.winter.NEM = fitlm(data.winter,'NEM_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.spring.NEM = fitlm(data.spring,'NEM_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.summer.NEM = fitlm(data.summer,'NEM_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% allSites_mdl.fall.NEM = fitlm(data.fall,'NEM_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + site');
% 
% % (2) Across all seasons
% % GPP
% allSites_mdl.all.GPP = fitlm(data.all,'GPP_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + season*site'); % Include interactions
% % ER
% allSites_mdl.all.ER = fitlm(data.all,'ER_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + season*site'); % Include interactions
% % NEM
% allSites_mdl.all.NEM = fitlm(data.all,'NEM_avg ~ tidal + wspd + temperature + salinity + chla + turbidity + season*site'); % Include interactions
