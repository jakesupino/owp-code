...
switch site
    case 'gull'
        depth1_avg = mean(means1.mean_depth([1 2 3:8 10 11])); % BC: Dep# 1, 2, 5-10, 12, 13
        depth2_avg = mean(means2.mean_depth([1:6 8:10])); % ERDC: Dep# 1, 2, 5-8, 10-12
    case 'north'
        depth1_avg = mean(means1.mean_depth([1:6 8, 10])); % BC: Dep# 2, 6-10, 12, 14
        depth2_avg = mean(means2.mean_depth([1:4 7, 8, 10])); % ERDC: Dep# 2, 6-8, 11, 12, 14
    case 'south'
        depth1_avg = mean(means1.mean_depth([1:7 9:11])); % BC: Dep# 1, 2, 4-8, 10-12
        depth2_avg = mean(means2.mean_depth([1:6 9 10])); % ERDC: Dep# 1, 2, 5, 7-9, 12, 13
end
...
%===Remove depth offset============================================
depth1_offset = mean(sonde1.depth,"omitnan");
depth2_offset = mean(sonde2.depth,"omitnan");
delta_depth1 = abs(depth1_offset - depth1_avg);
delta_depth2 = abs(depth2_offset - depth2_avg);
...
[pks0,locs0] = findpeaks(smoothed0,'MinPeakDistance',120);
...
[pks1,locs1] = findpeaks(smoothed1,'MinPeakDistance',60);
[pks2,locs2] = findpeaks(smoothed2,'MinPeakDistance',60);
...
% Find time offset between first sonde peak and corresponding USGS peak
diff1 = abs(usgs.datetime_utc(locs0) - sonde1.datetime_utc(locs1(1)));
offset1 = min(diff1);

diff2 = abs(usgs.datetime_utc(locs0) - sonde2.datetime_utc(locs2(1)));
offset2 = min(diff2);

% Adjust UTC time
datetime_utc1_adj = sonde1.datetime_utc - offset1;
datetime_utc2_adj = sonde2.datetime_utc - offset2;

% Adjust local time
datetime_local1_adj = sonde1.datetime_local - offset1;
datetime_local2_adj = sonde2.datetime_local - offset2;
...
% with adjusted data (datetime_utc1_adj, datetime_utc2_adj, datetime_local1_adj, datetime_local2_adj, depth1_adj, depth2_adj, p1_adj, p2_adj)

% Variables to save:
% depth1_avg, depth2_avg, offset1, offset2