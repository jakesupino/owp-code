%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% dielAnalysis_montecarlo.m
% This script uses Monte Carlo simulations to estimate the mean and standard 
% deviation for GPP, ER, and NEM using the estimated errors for the input
% parameters.
%
% AUTHOR:
% Emily Chua 
...
% Define errors (σ = 1 standard deviation) for each term
...
% Load diel analysis results from different k parameterizations from Gull sensitivity analysis
...
%==========================================================================
counter = 10^4;         % Number of simulations
...
kparam_err(1) = mean(abs(diff([R2b.fas.k, W2b.fas.k],1,2)),'omitmissing');  % R&H-2008 vs W-2014
kparam_err(2) = mean(abs(diff([R2b.fas.k, Eb.fas.k],1,2)),'omitmissing');   % R&H-2008 vs E-2019
kparam_err(3) = mean(abs(diff([W2b.fas.k, Eb.fas.k],1,2)),'omitmissing');   % W-2014 vs E-2019

% Take largest difference between parameterizations
kparam_err = max(kparam_err)/2;   % [h-1]
...
for j = 1:counter
    ...
    kparam_randErr = randn*kparam_err;
    ...
    D(:,j) = (k+kparam_randErr).*(DOsat - (DOconc+DOconc_randErr)); % [mmol m-3 h-1]
end
toc
...
errorbar(dt_utc,D_avg,D_sd,'.','DisplayName','MC average')
hold on
...
title([site,': Original vs. Monte Carlo Results (Detided Data)'])

% Save errors in table
sigma_tbl = table(S_err,T_err,d_err,p_err,DOconc_err,slp_err,U10_err,kparam_err);
sigma_tbl.Properties.VariableUnits = {'psu','degC','m','dbar','umol/L','hPa','m/s','h-1'};
...
switch option
    case 'Yes'
        ...
        save('MonteCarloResults.mat','diel_dtd_MC','sigma_tbl','counter')
        disp('File saved!')
    case 'No'
        disp('File not saved.')
end

option = questdlg('Save the figure?','Save figure','Yes','No','Yes');
switch option
    case 'Yes'
        cd([rootpath,'figures\diel-analysis\monte-carlo'])
        saveas(fig1,[site,'-MonteCarloComparison.fig'])
        saveas(fig1,[site,'-MonteCarloComparison.png'])
        disp('Figure saved!')
    case 'No'
